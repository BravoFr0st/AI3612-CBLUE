...
class CRFClassifier(nn.Module):
    def __init__(self, hidden_size: int, num_labels: int, dropout: float):
        ...
        self.rnn_dim = 256
        self.birnn = nn.LSTM(hidden_size, self.rnn_dim, num_layers=2, bidirectional=True, batch_first=True)
        self.hidden2tag = nn.Linear(2 * self.rnn_dim + hidden_size, self.num_labels)

    
    def _alignment(self, list_args, value):
        # align labels to same length by padding value(0)
    ...
    def _pred_labels(self, hidden_states, input_mask):
        '''NOTE: This is where to modify for CRF.
        ...
        pred_label_list = self._alignment(self.crf.decode(emissions, input_mask.byte()), 0)
        pred_labels = torch.tensor(pred_label_list, device = "cuda:0")
    ...
    def forward(self, hidden_states, attention_mask, labels=None, no_decode=False, label_pad_token_id=NER_PAD_ID):    
        ...
        sequence_output, _ = self.birnn(hidden_states)
        out = torch.cat((hidden_states, sequence_output), 2)
        sequence_output = nn.functional.relu(out)
        loss, pred_labels = None, None
        
        if labels is None:
            pred_labels = self._pred_labels(sequence_output, attention_mask)    
        else:
            emissions = self.hidden2tag(sequence_output)
...
class BertForCRFHeadNestedNER(BertPreTrainedModel):
    ...
    def __init__(self, config: BertConfig, num_labels1: int, num_labels2: int):
        ...
        self.classifier1 = CRFClassifier(config.hidden_size, num_labels1, config.hidden_dropout_prob)
        self.classifier2 = CRFClassifier(config.hidden_size, num_labels2, config.hidden_dropout_prob)
        self.classifier3 = LinearClassifier(config.hidden_size, num_labels1, config.hidden_dropout_prob)
        self.classifier4 = LinearClassifier(config.hidden_size, num_labels2, config.hidden_dropout_prob)
    ...
    def forward(
        ...
        output1 = self.classifier1.forward(sequence_output, attention_mask, labels, no_decode=no_decode)
        output2 = self.classifier2.forward(sequence_output, attention_mask, labels2, no_decode=no_decode)
        output_c = _group_ner_outputs(output1, output2)

        output3 = self.classifier3.forward(sequence_output, labels, no_decode=no_decode)
        output4 = self.classifier4.forward(sequence_output, labels2, no_decode=no_decode)
        output_l = _group_ner_outputs(output3, output4)
        
        grouped_loss = None
        grouped_logits = None
        if not (output_c.loss is None or output_l.loss is None):
            grouped_loss = (output_c.loss + output_l.loss) / 2
        grouped_logits = output_c.logits

        output = NEROutputs(grouped_loss, grouped_logits)
        return output
